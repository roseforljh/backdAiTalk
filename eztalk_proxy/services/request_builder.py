...
# 格式化输出prompt - 用于所有大模型
# 根据前端EnhancedMarkdownText.kt解析规范优化
MARKDOWN_FORMAT_SYSTEM_PROMPT = """严格遵循以下Markdown格式规范来组织你的输出，确保前端能正确解析和渲染：
...
* 段落之间使用**两个换行符**分隔
* 句子结束后换行使用**一个换行符**
* 中英文之间添加适当空格：`这是 English 混排的例子`

### 文本强调
* **粗体文本：** 使用 `**文本**` 格式，必须用于重点内容
* *斜体文本：* 使用 `*文本*` 格式
* ~~删除线：~~ 使用 `~~文本~~` 格式
...
### 代码块（严格要求）
* **必须**使用三个反引号包裹：````语言\n代码内容\n````
* **必须**指定语言标识符（python、javascript、java、sql、bash等）
* 代码内容前后不能有多余的反引号
* 正确示例：
```python
def hello():
...
### 表格内容格式
* 表格单元格内可使用Markdown格式：**粗体**、*斜体*、`代码`、[链接](URL)、$数学公式$
* 示例：`| **功能** | `代码示例` | $x^2$ |`
...
1. **代码块必须完整**：确保有开始和结束的三个反引号
2. **数学公式必须包装**：所有数学表达式都要用美元符号包裹
3. **表格格式严格**：必须有表头分隔线，列对齐
4. **语言标识必须**：代码块必须指定编程语言
5. **空格规范**：标题、列表项后必须有空格

遵循这些规范可确保前端正确解析和美观渲染你的输出内容。"""
