    ...
    if active_messages_for_llm:
        ...
        if last_user_message_for_search:
            for part in last_user_message_for_search.parts:
                if isinstance(part, PyTextContentPart) and part.text: user_query_for_search_gemini += part.text.strip() + " "
                    ...
                    try:
                            ...
                            for candidate in chunk_json["candidates"]:
                                if "content" in candidate and "parts" in candidate["content"]:
                                    for part_data in candidate["content"]["parts"]:
                                        part_text = part_data.get("text"); is_thought = part_data.get("thought") is True 
                                        if part_text:
                        ...
                        if "promptFeedback" in chunk_json:
                            ...
                            if block_reason:
                                ...
                                if safety_ratings: error_message_for_client += f" 安全评级详情: {str(safety_ratings)[:100]}"
                                ...
                                return
                    except orjson.JSONDecodeError: logger.warning(f"{log_prefix}: (Gemini REST) Failed to parse SSE JSON: {sse_data_bytes.decode(errors='ignore')[:100]}")
                    except Exception as e_proc_sse: logger.error(f"{log_prefix}: (Gemini REST) Error processing SSE line: {sse_data_bytes.decode(errors='ignore')[:100]}, error: {e_proc_sse}", exc_info=True)
            if not final_finish_event_sent:
                if _had_any_reasoning_event_sent_in_stream and not _reasoning_finish_event_sent_flag:
                   yield await sse_event_serializer_rest(AppStreamEventPy(type="reasoning_finish", timestamp=get_current_time_iso()))
                final_finish_event_sent = True
